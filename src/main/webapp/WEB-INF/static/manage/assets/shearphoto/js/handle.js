window.ShearPhoto.MINGGE(function() {
    //██████████重要设置████████████████
    var relativeUrl = relativeUrl; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
    ...
    Shear.config({
        ...
        preview: [350], // 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组)     数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图,shearphoto性能越差！官方不建意你开启这个功能，尽可能请设为preview:false

		/*记住 preview 尽量设false*/

        url: actionUrl, //后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了
        ...
        proportional: [0, <!--截框的宽高比例（宽除以高的比例值，这个设置其实就是1，对！你可以直接写1  如填3/4 那么就是0.75的比例,不设比例请设为0，注意更改比例后，后端也要进行相应设置，否则系统会给你抱出错误-->
			 ...
			 */
            300, //必须整数！启动后的截框初始宽度(整数型，禁止含小数点)
        ...
        Max: 600, //一开始启动时，图片的宽和高，有时候图片会很大的，必须要设置一下(整数型，禁止含小数点)，尽可能和scopeWidth值 一致
    ...
    Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
        ...
        creatImg.innerHTML = '<strong><i></i>上传成功</strong><br/><a href="javascript:;" id="completeA">确定</a>';
        else
            creatImg.innerHTML = '<strong><i></i>上传失败，请稍后再试</strong><br/><a href="javascript:;" id="completeA">确定</a>';
        complete.appendChild(creatImg);
