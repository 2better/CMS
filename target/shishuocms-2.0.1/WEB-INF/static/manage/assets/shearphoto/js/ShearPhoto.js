...
window.ShearPhoto.prototype = {
	...
	HTML5: {
		...
		Effects: function(StrEvent, HTML5) {
			...
			return function() {
				...
				if (HTML5.Aclick === this) {
					this_.pointhandle(1500, 1, "", 2, "#307ff6", "#fff");
					return;
				}
				HTML5.lock = true;
				...
				this_.pointhandle(0, 1, "正在加载" + StrEvent + "效果！稍等....，不要动鼠标，可能有点卡", 2, "#fbeb61", "#3a414c", function() {
					setTimeout(function() {
						...
						delete DATA64;
						this_.pointhandle(1500, 1, StrEvent + "效果加载成功！提示：如果机器配置差，效果加载时间会更长", 1, "#307ff6", "#fff");
						HTML5.lock = false;
		...
		CanvasImg: function(SendArry, postArgs, this_) {
            ...
            readerForm.append("type", $("input[type='radio']:checked").val());
	...
	config: function(arg) {
		...
		arg.scope.style.width = arg.black.style.width = arg.SelectBox.style.width = 600 + "px";
		arg.scope.style.height = arg.black.style.height = arg.SelectBox.style.height = 350 + "px";
		...
		this.pointhandle(3e3, 10, "请选择图片", 2, "#307ff6", "#fff");
	},
	zoom: function() {
		var this_ = this,
	...
	SendPHP: function(postArgs) {
		var this_ = this,
			...
			ResultData;
		this.SendUserMsg("正在处理截图，稍等...", 0, 2, "#fbeb61", "#3a414c", true, true, function() {
			if ((HTML5.HTML5PHP || HTML5.PhotoHTML5True) && HTML5.canvas) {
				try {
					...
					this_.SendUserMsg("截图失败，请更换浏览器或者换张图片", 5e3, 0, "#f4102b", "#fff", false);
					return;
				}
			} else {
				var POSTHTML = "";
			...
			this_.MyAjax.carry({
				...
				success: function(serverdata) {
					...
					if (serverdata === false) {
						this_.SendUserMsg("", 5e3, 0, "#f4102b", "#fff", false);
						return;
					}
					if (serverdata["erro"]) {
				...
				error: function(ErroMsg) {
					this_.SendUserMsg("请求超时，请稍后再试", 5e3, 0, "#f4102b", "#fff", false);
				},
				cache: false
			});
		});
	...
	Upsubmit: function(upfile) {
		try {
			...
			if (files) {
				...
				if (files.size <= 0) {
					this.DelCreateUpfile();
					this.config.erro("不能选择空字节文件");
					return;
				}
				if (this.config.HTML5.canvas) {
					if (files.size > this.config.HTML5FilesSize * 1024 * 1024) {
						this.DelCreateUpfile();
						this.config.erro("上传图片不能大于" + this.config.HTML5FilesSize + "M");
						return;
					}
				} else if (files.size > this.config.FilesSize * 1024 * 1024) {
					this.DelCreateUpfile();
					this.config.erro("上传图片不能大于" + this.config.FilesSize + "M");
					return;
				}
			}
			var this_ = this;
			...
			this.DelCreateUpfile();
			this.config.erro("你选择了非图片类型，或图片路径有误");
			return;
		}
	},
	run: function(fun) {
